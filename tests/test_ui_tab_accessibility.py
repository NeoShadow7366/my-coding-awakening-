...
def test_flux_lora_hint_and_strength_clamp_wiring():
    ...
    assert "Mode chip action: switch to default display options. Shortcuts: M/O toggles mode, Shift+M resets display options, C/W enables compact mode, S toggles row hints, P toggles preserved indicators, X/Z toggles compact mismatch badges, B toggles row clear buttons, I toggles hide incompatible, L toggles the family legend, K/? shows shortcut help, F/End focuses mode chip, G/Home focuses first option, D/R/0/Backspace enable default mode, H/T toggles this panel, Esc closes this panel." in js
    assert "Mode chip action: switch to compact display options. Shortcuts: M/O toggles mode, Shift+M resets display options, C/W enables compact mode, S toggles row hints, P toggles preserved indicators, X/Z toggles compact mismatch badges, B toggles row clear buttons, I toggles hide incompatible, L toggles the family legend, K/? shows shortcut help, F/End focuses mode chip, G/Home focuses first option, D/R/0/Backspace enable default mode, H/T toggles this panel, Esc closes this panel." in js
    assert "const hotkey = (event.key || '').toLowerCase();" in js
    assert "const isEscape = event.key === 'Escape';" in js
    assert "if (hotkey !== 'm' && hotkey !== 'o' && hotkey !== 'c' && hotkey !== 'w' && hotkey !== 'd' && hotkey !== 'r' && hotkey !== 'h' && hotkey !== 't' && hotkey !== 's' && hotkey !== 'p' && hotkey !== 'x' && hotkey !== 'z' && hotkey !== 'b' && hotkey !== 'i' && hotkey !== 'l' && hotkey !== 'k' && hotkey !== '?' && hotkey !== 'f' && hotkey !== 'end' && hotkey !== 'g' && hotkey !== 'home' && hotkey !== '0' && hotkey !== 'backspace' && !isEscape) return;" in js
    ...
    assert "showToast('Display shortcuts: M/O mode, Shift+M reset, C/W compact, S hints, P preserved, X/Z mismatch, B row clear, I hide incompatible, L legend, K/? help, F/End focus mode chip, G/Home focus first option, D/R/0/Backspace default, H/T toggle panel, Esc close.', 'pos');" in js
    ...
    assert "if (hotkey === 'd' || hotkey === 'r' || hotkey === '0' || hotkey === 'backspace') {" in js
