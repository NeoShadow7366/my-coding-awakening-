...
def test_flux_lora_hint_and_strength_clamp_wiring():
    ...
    assert "if (hotkey !== 'm' && hotkey !== 'c' && hotkey !== 'd' && hotkey !== 'r' && hotkey !== 'h' && hotkey !== 's' && hotkey !== 'p' && hotkey !== 'x' && hotkey !== 'b' && hotkey !== 'i' && hotkey !== 'l' && hotkey !== 'k' && hotkey !== '?' && hotkey !== 'f' && hotkey !== 'g' && hotkey !== '0' && hotkey !== 'backspace' && !isEscape) return;" in js
    ...
    assert "showToast('Display shortcuts: M mode, Shift+M reset, C compact, S hints, P preserved, X mismatch, B row clear, I hide incompatible, L legend, K/? help, F focus mode chip, G focus first option, D/R/0/Backspace default, H toggle panel, Esc close.', 'pos');" in js
