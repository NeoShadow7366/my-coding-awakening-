...
def test_flux_lora_hint_and_strength_clamp_wiring():
    ...
    assert "Mode chip action: switch to default display options. Shortcuts: M toggles mode, Shift+M resets display options, C enables compact mode, S toggles row hints, P toggles preserved indicators, X toggles compact mismatch badges, D/R enable default mode, H toggles this panel, Esc closes this panel." in js
    assert "Mode chip action: switch to compact display options. Shortcuts: M toggles mode, Shift+M resets display options, C enables compact mode, S toggles row hints, P toggles preserved indicators, X toggles compact mismatch badges, D/R enable default mode, H toggles this panel, Esc closes this panel." in js
    assert "const hotkey = (event.key || '').toLowerCase();" in js
    assert "const isEscape = event.key === 'Escape';" in js
    assert "if (hotkey !== 'm' && hotkey !== 'c' && hotkey !== 'd' && hotkey !== 'r' && hotkey !== 'h' && hotkey !== 's' && hotkey !== 'p' && hotkey !== 'x' && !isEscape) return;" in js
