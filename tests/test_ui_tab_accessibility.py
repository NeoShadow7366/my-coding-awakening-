...
def test_flux_lora_hint_and_strength_clamp_wiring():
    ...
    assert "showToast('Display shortcuts: M/O mode, Shift+M reset, C/W compact, S/N hints, P/J preserved, X/Z mismatch, B/V row clear, I/U hide incompatible, L/Y legend, K/? help, F/End focus mode chip, G/Home focus first option, D/R/A/0/Backspace default, H/T toggle panel, Esc close.', 'pos');" in js
    ...
    assert "if (hotkey === 'd' || hotkey === 'r' || hotkey === 'a' || hotkey === '0' || hotkey === 'backspace') {" in js
